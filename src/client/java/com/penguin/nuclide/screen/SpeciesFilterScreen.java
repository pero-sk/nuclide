...
public class SpeciesFilterScreen extends Screen {
    ...
    private static final int ENTRY_CLICK_HEIGHT = 80;
    ...
    public SpeciesFilterScreen(Hand hand) {
        super(Text.literal("Select Filter Species"));
        this.hand = hand;
        loadAllSpecies();
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int index = 0; index < entries.size(); index++) {
            ...
            if (entryBottom >= visibleTop && entryTop <= visibleBottom) {
                boolean hovered = isMouseOverEntry(mouseX, mouseY, entryTop);
                if (hovered) {
                    context.fill(16, entryTop, this.width - 16, entryTop + ENTRY_CLICK_HEIGHT, 0x33FFFFFF);
                }
    ...
    private boolean isMouseOverEntry(double mouseX, double mouseY, int entryTop) {
        return mouseX >= 16
                && mouseX <= this.width - 16
                && mouseY >= entryTop
                && mouseY <= entryTop + ENTRY_CLICK_HEIGHT;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) {
            ...
            for (Entry entry : entries) {
                if (isMouseOverEntry(mouseX, mouseY, y) && !entry.failed()) {

                    ClientPlayNetworking.send(new SetSpeciesFilterC2SPacket(hand, entry.id()));
                    close();
                    return true;
                }
    ...
    private Entry loadEntry(SpeciesDefinition species) {
        ...
        try {
            ...
            return new Entry(id, displayName, null, true, e.getClass().getSimpleName());
        }
    }
